...
export default function CategoryForm({ slug }: { slug?: string[] }) {
  ...
  const [filters, setFilters] = useState<
    Omit<QueryServiceCategoryDto, "page" | "limit">
  >({
    search: "",
    isActive: undefined,
  });
  ...
  const validateForm = () => {
    ...
    const displayOrder = formData.displayOrder
      ? parseFloat(formData.displayOrder)
      : null;
    if (
      displayOrder !== null &&
      (isNaN(displayOrder) ||
  ...
  const handleSave = async (data: CategoryDetailFormData) => {
    ...
    if (dialogMode === "add") {
      await createMutation.mutateAsync(
        submissionData as CreateServiceCategoryDto
      );
                ...
                <TableRow>
                  <TableCell colSpan={6} align="center" sx={{ py: 10 }}>
                    <Alert severity="error">
                      Failed to load categories. Please check console for
                      details.
                    </Alert>
                  </TableCell>
                </TableRow>
