...
import { CustomerType, CustomerStatus, Gender, CreateCustomerDto } from "@/types/customer";
import {
...
import { CustomerFormDialog } from "../customers/CustomerForm";
...
export default function BookingsPage() {
  ...
  const [customerFormDialogMode, setCustomerFormDialogMode] = useState<'add' | 'edit' | 'view'>('add');
  const [customerFormCustomerId, setCustomerFormCustomerId] = useState<number | null>(null);
  ...
  const fetchProducts = async () => {
    try {
      // Tùy thuộc vào cách getProducts trả về data, bạn cần điều chỉnh
  ...
  }, [page, rowsPerPage, filterStatus, filterStore, filterDate, customers.length, staff.length, stores.length, products.length, services.length]);
  ...
  const addItem = () => {
    if (
      ...
      const newItem: InvoiceItem = {
        ...
        totalPrice: (parseFloat(currentItem.unit_price) * parseInt(currentItem.quantity)) - parseFloat(currentItem.discount),
        staffId: currentItem.staff_id ? parseInt(currentItem.staff_id) : null,
        staff_name: staff.find((s) => s.id.toString() === currentItem.staff_id)?.full_name,
      };
  ...
  return (
    <>
      ...
      >
        ...
        <MenuItem
          onClick={() => selectedBooking && handleStartService(selectedBooking.id)}
        ...
        <MenuItem
          onClick={() => selectedBooking && handleCompleteService(selectedBooking.id)}
        ...
        <DialogContent dividers>
          {dialogMode === "view" && selectedBooking ? (
            ...
            <Grid container spacing={3} sx={{ mt: 0.5 }}>
              ...
              {bookingItems.length > 0 && dialogMode !== "view" ? (
                <Grid size={{ xs: 12 }} sx={{ mb: 3 }}>
                  <List component={Paper} variant="outlined">
                    {/* Header Button to Toggle Collapse */}
                    <ListItemButton onClick={() => setItemsOpen(!itemsOpen)} sx={{ py: 1.5 }}>
                      <ListItemIcon>
                        <Inventory />
                      </ListItemIcon>
                      <ListItemText
                            ...
                            <ListItemText
                              ...
                              secondary={
                                <Box sx={{ display: 'flex', gap: 1, mt: 0.5, flexWrap: 'wrap' }}>
                                  ...
                                  />
                                  <Chip
                                    label={`Unit Price: ${formatCurrency(item.unitPrice)}`}
                                    size="small"
                                    variant="outlined"
                                  />
                                  {item.staffId && (
                                    <Chip
                                      label={`Staff: ${item.staff_name || staff.find(s => s.id === item.staffId)?.full_name || "N/A"}`}
                                      size="small"
                                      color="default"
                                    />
                                  )}
                                  {item.discount > 0 && (
                                    <Chip
                                      label={`Discount: -${formatCurrency(item.discount)}`}
                                      size="small"
                                      color="success"
                                      variant="outlined"
                                    />
                ...
                <Grid size={{ xs: 12 }} sx={{ mb: 3 }}>
                  <Alert severity="info">Add services or products to this booking.</Alert>
                </Grid>
              ) : null}

              {/* Invoice Items Section for Edit Mode (The original LIST format for VIEW mode ONLY) */}
              {isItemsLoading ? (
                <Grid size={{ xs: 12 }} sx={{ textAlign: "center", my: 3 }}>
                  <CircularProgress />
                  <Typography>Loading items...</Typography>
                            ...
                            <ListItemText
                              primary={
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <Typography variant="body1" fontWeight="600" sx={{ maxWidth: '70%' }}>
                                    {item.itemName}
                                  </Typography>
                                  <Typography
                                    variant="body1"
        ...
        <DialogContent dividers>
          ...
          {bookingItems.map((item, index) => (
            <Box key={index} sx={{ mb: 2, p: 2, border: '1px solid #ddd', borderRadius: 1 }}>
              <Typography>{item.itemName} - {formatCurrency(item.unitPrice)} x {item.quantity}</Typography>
            </Box>
          ))}
          <Typography variant="h6">
            Total: {formatCurrency(bookingItems.reduce((sum, item) => sum + item.totalPrice, 0))}
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                await completeService(selectedBooking!.id, invoiceData as CompleteServicePayload);
                await loadData();
            ...
            sx={{ bgcolor: SUCCESS_COLOR, "&:hover": { bgcolor: SUCCESS_COLOR } }}
          >
      ...
      {customerFormDialogOpen && (
        <CustomerFormDialog 
          ...
          onClose={() => setCustomerFormDialogOpen(false)}
          onSuccess={async (message, severity) => {
            setSnackbar({ open: true, message: message, severity: severity || 'success' });
            ...
            if (severity !== 'error' && selectedBooking) {
              try {
                ...
                setSnackbar({
                  open: true,
                  message: "Customer created/updated & Service started successfully!",
                  severity: "success",
                });
              } catch (error) {
                setSnackbar({
                  open: true,
                  message: "Customer updated, but failed to start service.",
                  severity: "error",
                });
              }
            }
